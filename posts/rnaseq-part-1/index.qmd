...
We load in our fastq files and reference genome into the nf-core RNA-seq pipeline, which will perform all necessary steps to generate a count matrix. The pipeline can be run on a local machine or on a high-performance computing cluster. The nf-core pipeline is built using Nextflow, which allows for easy parallelization and reproducibility.
...
``` bash
# Install nf-core RNA-seq pipeline
# This requires Nextflow to be installed on your system
